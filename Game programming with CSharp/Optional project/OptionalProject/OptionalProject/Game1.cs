...
namespace OptionalProject
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            this.burger = new Burger(Content, "burger", GameConstants.WINDOW_WIDTH / 2, GameConstants.WINDOW_HEIGHT - 10);
            SpawnBear();
        }
        ...
        private void SpawnBear()
        {
            ...
            int x = GetRandomLocation(SPAWN_BORDER_SIZE, GameConstants.WINDOW_WIDTH - SPAWN_BORDER_SIZE * 2);
            int y = GetRandomLocation(SPAWN_BORDER_SIZE, GameConstants.WINDOW_HEIGHT - SPAWN_BORDER_SIZE * 2);

            // generate random velocity
            float speed = GameConstants.MIN_BEAR_SPEED + RandomNumberGenerator.NextFloat(GameConstants.BEAR_SPEED_RANGE);
            double angle = Math.PI * RandomNumberGenerator.NextDouble();
            Vector2 vector = new Vector2(speed * (float)Math.Cos(angle), speed * (float)Math.Sin(angle));

            // create new bear
            TeddyBear newBear = new TeddyBear(this.Content, "teddybear", x, y, vector);
